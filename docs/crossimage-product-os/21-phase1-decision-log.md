...
| P-17 | **初回提案の価格提示方針**（v1.5追記・オーナーフィードバック2026-08-14） | 工場実額は事前不明のため、初回3案の概算は「市場相場・高め側」で提示し、後からスタッフが実額で修正できること。具体化: ①承認画面で価格レンジを直接編集可能に（Loop Optionでは実装済みの編集UIを初回提案へも展開）②相場係数を高め側へ（CALIBRATION_VALUE）③案件単位で「金額非表示（工場確認後にご提示）」を選べるモード。断定価格禁止原則（05番）と整合 | 次増分 |
| P-18 | **実データ並走テスト未実施** | デモデータのみで実案件検証ゼロ。実案件1〜2件を現行運用（LINE/WeChat）と並走で入力し、摩擦点を収集して仕様を校正する。「今の仕様がベストか」の判断はこの実測後 | 次増分〜 |
| P-19 | **会話チャネルとの棲み分け（ブラックボックス化対策）**（オーナーフィードバック2026-08-14） | 顧客・中国側は今後もLINE/WeChat/Slackが楽 → 会話をシステムへ強制移行しない。使い方モデル=**「会話はチャット、決定と記録はシステム」**: 決定点（提案選択・条件合意・サンプル承認・検収）と状態・履歴・資料だけをシステムに置く。スタッフ向け「30秒記録」（チャット要点+画像を案件タイムラインへ貼る軽量UI）を先行実装し、LINE連携自動取込は後続検討 | 次増分 |
| P-20 | **資料アップロード**（オーナーフィードバック2026-08-14） | 顧客資料も中国側資料も案件へアップロードできること。documentsテーブルは15番設計済み・UI未実装。顧客画面と管理画面の両方に添付UIを実装し、タイムラインへ記録 | 次増分 |
| P-21 | **自社ホームページとの結合**（オーナー質問2026-08-14） | 第1段階=HPの問い合わせボタンのリンク先を本システムの相談ページにする（公開URL発行後・作業ゼロ）。第2段階=未ログインでも使える公開受付ページ（相談送信→リード化→スタッフがアカウント発行して本案件化）。第2段階はスパム対策とリード管理設計が前提 | 公開URL発行後 |
...
| v1.5 | 2026-08-14 | オーナーフィードバック（目的の再確認: ①自社作業の削減・自動運用 ②顧客育成〔依頼方法を教える/基準の立て方をガイドする/触りたくなる画面〕）を受け、P-17〜P-20を追記: 初回提案の価格提示方針（高め表示+スタッフ編集+非表示モード）/ 実データ並走テスト / 会話チャネル棲み分け「会話はチャット、決定と記録はシステム」+30秒記録 / 資料アップロードUI |
