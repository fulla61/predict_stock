 ...
 *
 * 生成物:
 *   {案件フォルダ}/{YYYYMMDD}_{発注書No}_{区分}請求書_{案件名}       … Google Sheet（編集原本）
 *   {案件フォルダ}/{YYYYMMDD}_{発注書No}_{区分}請求書_{案件名}.pdf   … 送付版
 *
 * 【書式】請求書は automation/invoice_format/FORMAT.md の「ピンク書式」を正とする。
 *   INVOICE_TEMPLATE_ID に登録する Google Sheet テンプレートは必ずこの書式で作成すること。
 *
 * 【重要】金額は機械が決められない。
...
function generateInvoice(row) {
  ...
  // 統一命名（2026-08-24 確定）: {YYYYMMDD}_{発注書No}_{区分}請求書_{案件名}
  var doc = (row.kind === '前金' || row.kind === '残金') ? row.kind + '請求書' : '請求書';
  var baseName = [ymdCompact(issueDate), row.poNo, doc, safeName(row.project)].join('_');
